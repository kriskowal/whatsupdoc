...
/**
 * @param text {String} a string starting with a curly brace
 * with a matching, nested curly brace somewhere before the
 * end.
 * @param node {Object} an object with an `errors` array.
...
exports.parseCurly = function (text, node) {
    ...
    if (nextClose < 0) {
        node.errors.push("Unmatched `{` in `@param` " + UTIL.enquote("{" + text));
        return ["", ""];
    } else if (nextOpen < 0) {
