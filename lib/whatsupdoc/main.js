...
exports.main = function (SYSTEM) {
    ...
    var api = options.args.shift() || "api";
...
exports.search = function (options) {
    ...
    if (options.engines.length)
        engine = {"engines": options.engines};
    else
        engine = ENGINE;

    var packages;
    if (options.prefixes.length)
        packages = PACKAGES.load(options.prefixes);
    else
        packages = PACKAGES;

    var finds = SEARCH.searchAll(engine, packages);
    if (!options.all) {
...
/**
 */
exports.makeHtml = function (node, directory, options) {
    ...
    var context = THATSALLFOLKS.Context(node, {
        "sourceLinks": options.sourceLinks,
        "showErrors": options.showErrors
    });
    path.write(template.format(node), {"charset": "UTF-8"});
};
